...
###################################################################
    ...
    a1,b1 = st.columns(2)
    name2 = a1.text_input('Enter Your Name')
    mail2 = b1.text_input('Enter Your Mail-id')
    msg2 =  st.text_area('Write a Message',max_chars=200)
    butt = st.button('Send')
    if butt and name2 and mail2 and msg2:
          succ = st.success('Messege sent')
          time.sleep(3)
          succ.empty()    
    # st.link('dss.com')
connection.commit()
